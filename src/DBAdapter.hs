...
-- | Check if this current target has changed
checkTargetHash :: TargetID -> Hash -> (Connection -> IO Bool)
checkTargetHash targetID newHash conn = do 
    result <- query conn checkHash (targetID, newHash)
    let (Only count) = head result :: Only Int
    return $ count == 0
  where checkHash = "SELECT COUNT() "
                 <> "FROM Targets "
                 <> "WHERE TargetID = ? AND Hash = ?"

-- | Update the hash for the given targetID
updateTargetHash :: TargetID -> Hash -> (Connection -> IO Bool)
updateTargetHash targetID newHash conn = do
    execute conn updateHash (newHash, targetID)
    isSuccessful conn
  where updateHash = "UPDATE Targets "
                  <> "SET Hash = ? "
                  <> "WHERE TargetID = ?"

-- | Update the content for the given targetID
updateTargetContent :: TargetID -> String -> (Connection -> IO Bool)
updateTargetContent targetId newContent conn = do
    execute conn updateHash (newContent, targetId)
    isSuccessful conn
  where updateHash = "UPDATE Targets "
                  <> "SET Content = ? "
                  <> "WHERE TargetID = ?"
...
addTarget target conn = do execute conn insertTarget target
                           isSuccessful conn
  where insertTarget = "INSERT INTO Targets (UserID, WebsiteID, Selector, Hash, Content) "
                    <> "VALUES (?,?,?,?,'')"
...
getTarget targetId conn = do
    ...
    return $ listToMaybe result
  where lookupTarget = "SELECT TargetID, UserID, WebsiteID, Selector, Hash, Content "
                    <> "FROM Targets "
                    <> "WHERE TargetID = ?"
